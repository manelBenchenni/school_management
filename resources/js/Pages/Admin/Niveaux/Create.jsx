...
export default function Create() {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('admin.niveaux.store'));
    };

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold tracking-tight text-slate-800">Add Niveau</h2>}>
            <Head title="Add Niveau" />
                ...
                <Card>
                    <form onSubmit={submit} className="space-y-5">
                        ...
                        <div>
                            ...
                            <TextInput id="label" value={data.label} placeholder="e.g. 3eme Annee Moyenne" onChange={(e) => setData('label', e.target.value)} />
                            <InputError message={errors.label} />
                        </div>

                        <div className="flex items-center justify-end gap-3 border-t border-slate-100 pt-5">
                            <Link href={route('admin.niveaux.index')}><SecondaryButton type="button">Cancel</SecondaryButton></Link>
                            <PrimaryButton disabled={processing}>Add Niveau</PrimaryButton>
                        </div>
                    </form>
                </Card>
            </div>
